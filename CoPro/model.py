...
class CoPro(torch.nn.Module):
    ...
    def get_model(self, n_ctx, n_pro, n_ctx_ab, n_pro_ab, class_name, backbone, pretrained_dataset):
        ...
        visual_gallery1_ab = torch.zeros((self.shot*self.grid_size[0]*self.grid_size[1], self.model.visual.embed_dim))
        self.register_buffer("feature_gallery1_ab", visual_gallery1_ab)
        ...
        visual_gallery2_ab = torch.zeros((self.shot*self.grid_size[0]*self.grid_size[1], self.model.visual.embed_dim))
        self.register_buffer("feature_gallery2_ab", visual_gallery2_ab)
        ...
        if self.precision == 'fp16':
            ...
            self.feature_gallery1_ab  = self.feature_gallery1_ab.half()
            self.feature_gallery2_ab  = self.feature_gallery2_ab.half()
    ...
    def enqueue_seg(self,abnormal_features):
        ...
        self.queue_ptr[0] = ptr
    @torch.no_grad()
    def dequeue_seg(self,ab_ahchor,n_ahchor):
        samepic_res=[]
        differentpic_res=[]
        ab_ahchor=ab_ahchor.repeat(225,1).reshape(1,144000)
        n_ahchor=n_ahchor.repeat(225,1).reshape(1,144000)
        if self.shot>1:
            for i in range(self.shot):
                samepic_feature = self.queue[i]
                samepic_score=criterion_tip(samepic_feature,ab_ahchor,n_ahchor)
                samepic_res.append(samepic_feature[torch.argmin(samepic_score)].unsqueeze(0))
                differentpic_feature = torch.cat([self.queue[:i],self.queue[i+1:]],dim=0)
                differentpic_feature = differentpic_feature.reshape(-1,144000)
            ...
            return torch.cat(samepic_res).reshape(self.shot,225,640),torch.cat(differentpic_res).reshape(self.shot,225,640)
        else :
            samepic_score=criterion_tip(self.queue.squeeze(0),ab_ahchor,n_ahchor)
            return self.queue[:,torch.argmax(samepic_score),:].reshape(self.shot,225,640)

    @torch.no_grad()
    def dequeue(self,ab_ahchor,n_ahchor):
        samepic_res=[]
        differentpic_res=[]
        if self.shot>1:
            for i in range(self.shot):
    ...
    def build_anom_image_feature_gallery(self, features1, features2):
        b1, n1, d1 = features1.shape
        self.feature_gallery1_ab.copy_(F.normalize(features1.reshape(-1, d1), dim=-1))

        b2, n2, d2 = features2.shape
        self.feature_gallery1_ab.copy_(F.normalize(features2.reshape(-1, d2), dim=-1))
