...
def load_deeppcb(category, k_shot, seed):
    ...
    assert category in deeppcb_classes

    test_img_path = os.path.join(DeepPCB_DIR, category, 'test')
    train_img_path = os.path.join(DeepPCB_DIR, category, 'train')
    ground_truth_path = os.path.join(DeepPCB_DIR, category, 'test')
    ...
    seed_file_dir = os.path.join('./datasets/seeds_deeppcb', category)
    ...
    if os.path.exists(seed_file_path):
        ...
        with open(seed_file_path, 'w') as f:
            f.write(f'#{k_shot}: ')
            f.write(' '.join(f'{idx:04d}' for idx in train_img_tot_paths[k]))
            f.write('\n')
