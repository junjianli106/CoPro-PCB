...
def get_dir_from_args(TASK, root_dir, **kwargs):
    ...
    prompt_desc = kwargs['prompt_desc']

    if prompt_desc:
        csv_dir = os.path.join(root_dir, f'{dataset}_desc{k_shot}', f'k_{k_shot}', 'csv')
        check_dir = os.path.join(root_dir, f'{dataset}_desc{k_shot}', f'k_{k_shot}', 'checkpoint')
        img_dir = os.path.join(root_dir, f'{dataset}_desc{k_shot}', f'k_{k_shot}', 'imgs')
